...
def main(argv):
    ...
    parser.add_argument("--fig_filename_pattern",
                        ...
                        type=str,
                        default="../../figures/EJT178_implant1/recording6_29-03-2022/{:08d}_var_mean_derivatives.{{:s}}")
    ...
    var_mean_derivatives = grad["variational_mean"]
    n_latents = var_mean_derivatives.shape[0]

    [perc_1, perc_99] = jnp.percentile(var_mean_derivatives, jnp.array([1, 99]))

    fig = go.Figure()
    for k in range(n_latents):
        trace = go.Histogram(x=var_mean_derivatives[k, :, :].flatten(), name=f"latent {k}")
        fig.add_trace(trace)
    fig.update_xaxes(title="Derivative", range=(perc_1, perc_99))
    fig.update_yaxes(title="Count")
    fig.update_layout(title=f"Number of Parameters: {var_mean_derivatives.size}")
