...
def main(argv):
    ...
    parser.add_argument("--fig_filename_pattern",
                        ...
                        type=str,
                        default="../../figures/EJT178_implant1/recording6_29-03-2022/{:08d}_C_derivatives.{{:s}}")
    ...
    C_derivatives = grad["C"]
    n_latents = C_derivatives.shape[1]

    [perc_1, perc_99] = jnp.percentile(C_derivatives, jnp.array([1, 99]))

    fig = go.Figure()
    for k in range(n_latents):
        trace = go.Histogram(x=C_derivatives[:, k], name=f"latent {k}")
        fig.add_trace(trace)
    fig.update_xaxes(title="Derivative", range=(perc_1, perc_99))
    fig.update_yaxes(title="Count")
    fig.update_layout(title=f"Number of Parameters: {C_derivatives.size}")
