...
def main(argv):
    ...
    variational_mean_scale = 1.0/jnp.sqrt(float(est_init["optim_params"]["variational_mean_curvature"]))
    variational_chol_vecs_scale = 1.0/jnp.sqrt(float(est_init["optim_params"]["variational_chol_vecs_curvature"]))
    C_scale = 1.0/jnp.sqrt(float(est_init["optim_params"]["C_curvature"]))
    d_scale = 1.0/jnp.sqrt(float(est_init["optim_params"]["d_curvature"]))
    kernels_params_scale = 1.0/jnp.sqrt(float(est_init["optim_params"]["kernels_params_curvature"]))
    ind_points_locs_scale = 1.0/jnp.sqrt(float(est_init["optim_params"]["ind_points_locs_curvature"]))

    params_scale = dict(
        variational_mean=jnp.ones_like(variational_mean) * variational_mean_scale,
        variational_chol_vecs=jnp.ones_like(variational_chol_vecs) * variational_chol_vecs_scale,
        kernels_params = [kernel_param0 * kernels_params_scale for kernel_param0 in kernels_params0],
        ind_points_locs = jnp.ones_like(ind_points_locs) * ind_points_locs_scale,
    )
