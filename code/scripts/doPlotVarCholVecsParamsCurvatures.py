...
def main(argv):
    ...
    parser.add_argument("--fig_filename_pattern",
                        ...
                        type=str,
                        default="../../figures/EJT178_implant1/recording6_29-03-2022/{:08d}_var_chol_vecs_curvatures.{{:s}}")
    ...
    var_chol_vecs_curvatures = curvatures["variational_chol_vecs"]
    n_latents = var_chol_vecs_curvatures.shape[0]

    [perc_1, perc_99] = jnp.percentile(var_chol_vecs_curvatures, jnp.array([1, 99]))

    fig = go.Figure()
    for k in range(n_latents):
        trace = go.Histogram(x=var_chol_vecs_curvatures[k, :, :, 0].flatten(), name=f"latent {k}")
        fig.add_trace(trace)
    fig.update_xaxes(title="Curvature", range=(perc_1, perc_99))
    fig.update_yaxes(title="Count")
    fig.update_layout(title=f"Number of Parameters: {var_chol_vecs_curvatures.size}")
