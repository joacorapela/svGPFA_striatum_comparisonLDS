...
def main(argv):
    ...
    # plot results
    fig = go.Figure()
    for i in range(len(run_cor)):
        trace = go.Scatter(x=run_cor_times[i], y=run_cor[i], name=f"latent {latent_indices[i]}")
        fig.add_trace(trace)
    fig.update_xaxes(title="Time (sec)")
    fig.update_yaxes(title="Pearson Correlation Coefficient")
