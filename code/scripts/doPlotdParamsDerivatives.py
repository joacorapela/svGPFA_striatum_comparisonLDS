...
def main(argv):
    ...
    parser.add_argument("--fig_filename_pattern",
                        ...
                        type=str,
                        default="../../figures/EJT178_implant1/recording6_29-03-2022/{:08d}_d_derivatives.{{:s}}")
    ...
    d_derivatives = grad["d"]

    [perc_1, perc_99] = jnp.percentile(d_derivatives, jnp.array([1, 99]))

    fig = go.Figure()
    trace = go.Histogram(x=d_derivatives[:, 0])
    fig.add_trace(trace)
    fig.update_xaxes(title="Derivative", range=(perc_1, perc_99))
    fig.update_yaxes(title="Count")
    fig.update_layout(title=f"Number of Parameters: {d_derivatives.size}")
